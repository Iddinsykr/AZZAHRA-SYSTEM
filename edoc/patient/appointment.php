...
<body>
    ...
    function calculateSessionTime($schedule_start_time, $appointment_number) {
    $start_time = strtotime($schedule_start_time);
    $duration_per_appointment = 40 * 60; // 10 minutes
    $break_start = strtotime('13:00:00');
    $break_end = strtotime('14:30:00');

    $current_time = $start_time;

    for ($i = 1; $i < $appointment_number; $i++) {
        $next_time = $current_time + $duration_per_appointment;
        if ($next_time >= $break_start && $next_time < $break_end) {
            $next_time = $break_end;
        }
        $current_time = $next_time;
    }

    return date("h:i A", $current_time);
}
    ...
    //TODO
    $sqlmain = "SELECT appointment.appoid, schedule.scheduleid, schedule.title, doctor.docname, patient.pname, schedule.scheduledate, schedule.scheduletime, appointment.apponum, appointment.appodate, appointment.approval_status FROM schedule 
...
$position_query = $database->query("SELECT COUNT(*) AS position 
    FROM appointment 
    WHERE scheduleid='$scheduleid' 
    AND appoid < '$appoid'");
$position_row = $position_query->fetch_assoc();
$position = $position_row['position'] + 1;

$schedule_query = $database->query("SELECT scheduletime FROM schedule WHERE scheduleid='$scheduleid'");
$schedule_row = $schedule_query->fetch_assoc();
$start_time = $schedule_row['scheduletime'];

$session_time = calculateSessionTime($start_time, $position);
                                            ...
                                            <td style="width: 25%;">
                                                    <div  class="dashboard-items search-items"  >
                                                        ...
                                                        <div class="h3-search">
                                                                ...
                                                                <div class="h4-search">
    Your Session Time: <b>'.$session_time.'</b>
</div>

    <div class="h4-search">
        Appointment Status: <b>' . $row["approval_status"] . '</b>
    </div>
